 ...
 * The final bufferRoute polygon is computed by forming a topological graph
...
public class OffsetCurveBuilder {
   ...
   * A value of 8 gives less than 2% max error in the bufferRoute distance.
  ...
  public List getLineCurve(Coordinate[] inputPts, double distance)
  {
    ...
    // a zero or negative width bufferRoute of a line/point is empty
    ...
    if (inputPts.length <= 1) {
      switch (endCapStyle) {
        ...
        case BufferOp.CAP_SQUARE:
          ...
          break;
          // default is for bufferRoute to be empty (e.g. for a butt line cap);
      }
    }
    else
...
//System.out.println(bufPt);
  ...
  private void addNextSegment(Coordinate p, boolean addStartPoint)
  {
    ...
    if (orientation == 0) { // lines are collinear
       ...
       * In this case we must add a offset joining curve to make sure the bufferRoute line
       * is continuous and tracks the bufferRoute correctly around the corner.
       * Note that the joining curve won't appear in the final bufferRoute.
       *
